...
--Q.4: Which city has the best customers? We would like to throw a promotional Music Festival in the city we made the
-------most money. Write a query that returns one city that has the highest sum of invoices totals.
...
--Q.5: Who is the best customer? The customer who has spent the most money will be declared the best customer. Write
...
--Q.1: Write query to return the email, first name, last name, & Genre of all Rock Music listners. Return your list
...
--Q.2: Let's invite the artists who have written the most rock music in our dataset. Write a query that returns the
...
-------miliseconds for each track. Order by the song length with the longest song listed first.
SELECT * FROM track;
...
--My solution-partially done
SELECT * FROM artist
WHERE artist.name IN(

SELECT name, SUM(total_spent) AS amt FROM (
	SELECT customer.customer_id, customer.first_name, customer.last_name, artist.name, SUM(invoice_line.unit_price * invoice_line.quantity) AS total_spent
    FROM invoice_line
    JOIN invoice ON invoice_line.invoice_id=invoice.invoice_id
    JOIN customer ON invoice.customer_id=customer.customer_id
